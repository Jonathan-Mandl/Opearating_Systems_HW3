...
void *producer(void *arg)
{
    ...
    for (int i = 0; i < num_products; i++)
    {
        ...
        sem_wait(&producer_empty[id]);
        // lock producer_mutex
        sem_wait(&producer_mutex[id]);
        // insert message to queue of producer.
        Producer_Queues[id]->insert(Producer_Queues[id], message);
        // unlock producer_mutex
        sem_post(&producer_mutex[id]);
        // up to counting semaphore of producer_full cells.
        sem_post(&producer_full[id]);
    }
    ...
    Producer_Queues[id]->insert(Producer_Queues[id], "DONE");
...
void *dispatcher()
{
    ...
    while (working_producers)
    {
        for (int i = 0; i < numProducers; i++)
        {
            ...
            // Check if the queue is producer_empty or not, and if it is producer_empty continue to next iteration of loop.
            if (Producer_Queues[i]->front == -1 || Producer_Queues[i]->front > Producer_Queues[i]->rear)
            {
                continue;
            }

            // decrement counting semaphores of producer_full cells
            sem_wait(&producer_full[i]);
            // lock producer_mutex.
            sem_wait(&producer_mutex[i]);

            // Queue has message, process it
            const char *message = Producer_Queues[i]->remove_message(Producer_Queues[i]);

            // unlock producer_mutex
            sem_post(&producer_mutex[i]);
            // up to empty cells semaphore
            sem_post(&producer_empty[i]);
            ...
            char *message_unedited = strdup(message);
            ...
            {
                sem_wait(&S_mutex);
                insert_unbounded(S_queue, message_unedited);
                sem_post(&S_mutex);
                sem_post(&S_full);
            }
            ...
            {
                sem_wait(&N_mutex);
                insert_unbounded(N_queue, message_unedited);
                sem_post(&N_mutex);
                sem_post(&N_full);
            }
            ...
            {
                sem_wait(&W_mutex);
                insert_unbounded(W_queue, message_unedited);
                sem_post(&W_mutex);
                sem_post(&W_full);
            }
            else
...
void *coEditor_N()
{
    while (1)
    {
        ...
        const char *message = remove_message_unbounded(N_queue);
        sem_post(&N_mutex);

        //if meesage is DONE, pass mesage and break from loop, without blocking
        if (strcmp(message, "DONE") == 0)
        {
        ...
        co_Editor_Queue->insert(co_Editor_Queue, message);
...
while (1)
    {
        ...
        sem_wait(&S_mutex);
        // remove message from dispatcher queue
        const char *message = remove_message_unbounded(S_queue);
        sem_post(&S_mutex);
...
void *coEditor_W()
{
       while (1)
        ...
        sem_wait(&W_mutex);
        // remove message from dispatcher queue
        const char *message = remove_message_unbounded(W_queue);
        sem_post(&W_mutex);
...
void *screen_manager()
{
    ...
    while(done_counter<3)
    {
        ...
        const char *message = co_Editor_Queue->remove_message(co_Editor_Queue);
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < numProducers; i++)
    {
        ...
        sem_init(&producer_mutex[i], 0, 1);
        sem_init(&producer_full[i], 0, 0);
        sem_init(&producer_empty[i], 0, queue_size);
    }

    // initialize dispatcher queues semaphores.
    sem_init(&N_mutex, 0, 1);
    sem_init(&N_full, 0, 0);
    ...
    sem_init(&co_editor_mutex,0,1);
    sem_init(&co_editor_full,0,0);
    sem_init(&co_editor_empty,0,coEditorQueueSize);
